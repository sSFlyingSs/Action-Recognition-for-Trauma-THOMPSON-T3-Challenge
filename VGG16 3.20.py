...
class VGGLSTM(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        # Replace final classifier layer with Identity so backbone outputs 4096-d features
        self.backbone.classifier[6] = nn.Identity()

        # Freeze all layers except the last two conv blocks (features[24:]) and classifier
...
def main():
    ...
    model_major = VGGLSTM(len(majority_classes)).to(device)
    model_minor = VGGLSTM(len(minority_classes)).to(device)
