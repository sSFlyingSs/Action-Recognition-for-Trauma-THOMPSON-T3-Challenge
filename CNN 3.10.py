...
# Custom CNN + LSTM Action Recognition
...
CLIP_LEN    = 16         # Number of frames per clip (fixed-length input for LSTM)
IMG_SIZE    = 112        # Reduced from 224 — lighter on memory and compute
BATCH_SIZE  = 8
EPOCHS      = 100
LR          = 1e-4
NUM_WORKERS = 6
...
# Output:         GlobalAvgPool → 256-d feature vector per frame
#
# Why this helps vs ResNet18:
#   • ~1.5 M params vs ~11 M  →  much less capacity to memorise training data
#   • Aggressive spatial dropout after every block adds strong regularisation
#   • No pretrained weights means no domain-mismatch fine-tuning instability
# ================================================================

class ConvBlock(nn.Module):
    """Two conv layers with BN + ReLU, one MaxPool, one Dropout."""

    def __init__(self, in_ch, out_ch, pool=True, drop=0.2):
        super().__init__()
        layers = [
...
class LightCNN(nn.Module):
    """
    Input : (B, 3, H, W)
    Output: (B, 256)  — flat feature vector ready for LSTM
    """

    def __init__(self, feat_dim=256):
        super().__init__()
        self.encoder = nn.Sequential(
            ConvBlock(3,   32,  pool=True,  drop=0.1),   # → 32 × H/2  × W/2
            ConvBlock(32,  64,  pool=True,  drop=0.2),   # → 64 × H/4  × W/4
            ConvBlock(64,  128, pool=True,  drop=0.3),   # → 128× H/8  × W/8
            ConvBlock(128, feat_dim, pool=True, drop=0.3),# → 256× H/16 × W/16
        )
...
class CNNLSTM(nn.Module):
    ...
    def __init__(self, num_classes, feat_dim=256):
        ...
        self.cnn = LightCNN(feat_dim=feat_dim)
...
def main():
    ...
    # Stronger augmentation to further fight overfitting
    ...
    model_major = CNNLSTM(len(majority_classes)).to(device)
    model_minor = CNNLSTM(len(minority_classes)).to(device)

    print(f"Major model params: {sum(p.numel() for p in model_major.parameters()):,}")
    print(f"Minor model params: {sum(p.numel() for p in model_minor.parameters()):,}")

    criterion = nn.CrossEntropyLoss(label_smoothing=0.1)

    optimizer_major = torch.optim.AdamW(model_major.parameters(), lr=LR, weight_decay=1e-3)
    optimizer_minor = torch.optim.AdamW(model_minor.parameters(), lr=LR, weight_decay=1e-3)
